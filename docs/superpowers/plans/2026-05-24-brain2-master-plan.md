...
2. **Sub-plan documents** — the actual task-by-task TDD plans. Filenames: `2026-05-24-brain2-plan-01-foundation.md` for the foundation; plans **02–14** under the `2026-05-25-brain2-plan-NN-<name>.md` prefix (the canonical series). **Status:** all plans **01–14 are written**. Implementation: 01–09 are implemented and their suites pass (run via the project venv — `.venv/bin/python -m pytest`); P10/Concepts is implemented (concept sync is a simplified stub — full LLM-driven ADD/UPDATE/SUPERSEDE/RETIRE/MERGE + supercession FSRS merge remain, see plan-10 deferrals); **plans 11–14 are authored but not yet executed**.
...
All 14 sub-plans are written. Plans 01–09 are implemented (passing suites) and P10/Concepts is implemented (with the simplified-sync caveat). Remaining **execution** work, in order: (1) close the plan-10 fidelity gaps (LLM-driven ADD/UPDATE/SUPERSEDE/RETIRE/MERGE sync + supercession FSRS merge); (2) execute **plan-11-reports**, **plan-12-interfaces**, **plan-13-ops-hardening**, **plan-14-postgres-store**; (3) revisit the two architectural items deferred from the fix pass — add-on namespaced storage (move Concepts/Reports off the direct `_conn`) and routing add-on events through the P04 outbox dispatcher (plan-09/plan-12 scope). Recommended approach: **subagent-driven development** — one fresh subagent per task with two-stage review. Always run tests via the project venv (`.venv/bin/python -m pytest`), not the bare `python` shim.
