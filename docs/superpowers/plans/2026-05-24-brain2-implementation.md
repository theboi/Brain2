...
> ⚠️ **SUPERSEDED (2026-05-24).** This Teams A–I / 3-phase plan was written before the Phase 4 (runtime scale & correctness) and Phase 5 (platform hardening) specs and the core/security least-privilege edits existed. It describes designs the current specs explicitly replace — a `core/` package layout (now `brain2/`); `store.emit_event` + an `EventQueue` processor (now the **transactional outbox** + `SKIP LOCKED` dispatch, [P4 §6](../specs/2026-05-24-brain2-phase4-scale-correctness.md)); startup-only orphan recovery + in-process `TaskRunner` (now a **worker fleet** with lease/heartbeat/sweeper, P4 §4); `CREATE TABLE IF NOT EXISTS` in `init_db()` (now a **schema-migration framework**, [P5 §2](../specs/2026-05-24-brain2-phase5-platform-hardening.md)); `WikiPage.path` as filesystem source of truth (now **wiki content in the DB**, P4 §9.4); and direct LLM-client use (now the **mandatory LLM gateway**, P4 §3). It also predates idempotency keys, keyset pagination, MCP agent identity, and connection discipline. **Do not execute this plan.** The authoritative build order is the [master plan](2026-05-24-brain2-master-plan.md) and its `plan-NN-*` sub-plans. Retained for the review trail only.
