/* Brain2 Console — shared minimal markdown renderer (headings, bold, code,
   wiki-links [[..]], citation footnotes [^n], blockquote, lists). */

function mdInline(s, key, onCite) {
  const parts = [];
  const re = /(\*\*[^*]+\*\*|\*[^*\n]+\*|`[^`]+`|\[\[[^\]]+\]\]|\[\^\d+\]|\[#\d+\])/g;
  let last = 0, m, i = 0;
  while ((m = re.exec(s))) {
...
function MiniMD({ text, onCite }) {
  const lines = (text || '').split('\n');
  const out = [];
  let list = [];
  const flush = (k) => { if (list.length) { out.push(<ul key={'u' + k} style={{ margin: '6px 0 12px', paddingLeft: 20, color: 'var(--fg-muted)' }}>{list}</ul>); list = []; } };
  lines.forEach((ln, i) => {
    ...
    else if (/^\[\^\d+\]:/.test(ln)) { flush(i); out.push(<div key={i} style={{ fontSize: 12, color: 'var(--fg-muted)', fontFamily: 'var(--mono-font)', margin: '2px 0', paddingTop: 6, borderTop: list.length ? 'none' : undefined }}>{ln}</div>); }
...
Object.assign(window, { MiniMD, mdInline });
