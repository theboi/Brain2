/* Brain2 Console — Agents app: shell, queue state + live simulation. */
...
function advance(w) {
  ...
  todos = todos.map((t) => {
    ...
    if (li >= 0 && msgs[li].role === 'assistant' && msgs[li].reveal != null) {
      ...
      m.reveal = Math.min(units, m.reveal + 3);
      if (nt.elapsed > 3) m.tools = (m.tools || []).map((tt) => tt.running ? { ...tt, running: false, result: tt.result || 'done' } : tt);
      msgs[li] = m; nt.messages = msgs;
    }
    if (nt.elapsed >= nt.dur) {
      const footer = { latency: (nt.dur / 10).toFixed(1) + 's', tokens: nt.loc === 'cloud' ? Math.round(600 + nt.dur * 12) + ' tok' : 'local', cost: nt.loc === 'cloud' ? '$' + (0.002 + nt.dur * 0.0003).toFixed(3) : 'local' };
      ...
      nt = { ...nt, status: 'done', messages: fmsgs, when: 'just now', tokens: footer.tokens, memoryFlushed: true, doneAt: Date.now() + Math.random() };
      agents = agents.map((a) => a.id === nt.agentId ? { ...a, status: 'idle', taskId: null } : a);
    }
    return nt;
  });
...
function AgentsApp() {
  ...
  const [world, setWorld] = React.useState({ agents: SEED_AGENTS, todos: SEED_TODOS });
  ...
  const actions = {
    ...
    rerun: (id) => setWorld((w) => {
      ...
      const um = s.messages.find((m) => m.role === 'user') || { role: 'user', text: s.title, by: s.by };
      const nt = { id: 'n' + Date.now(), title: s.title, by: s.by, priority: false, status: 'queued', agentId: null, modelPref: s.modelPref || (s.loc === 'cloud' ? 'cloud' : 'auto'), loc: s.loc, messages: [{ ...um }] };
    ...
    add: ({ title, assign, model }) => {
      ...
      const nt = { id: 'n' + Date.now(), title, by: 'alice', priority: false, status: 'queued', agentId: null, modelPref: model === 'auto' ? 'auto' : model, loc: rm.loc, preferredAgent: assign === 'any' ? null : assign, messages: [{ role: 'user', text: title, by: 'alice' }] };
  ...
  React.useEffect(() => { if (openId && !todos.find((t) => t.id === openId)) setOpenId(null); }, [todos, openId]);

  const chips = [['all', 'All', todos.length], ['running', 'Running', running.length], ['queued', 'Queued', queued.length], ['done', 'Done', done.length]];
  const showGroup = (g) => filter === 'all' || filter === g;
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <main style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 24px 14px', flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            <a href="Settings.html#models" style={{ ...agBtnGhost(), textDecoration: 'none' }}><Icon name="cpu" size={15} color="var(--fg-muted)" /> <span className="b2-hide-sm">Manage models</span><span className="b2-show-sm" style={{ display: 'none' }}>Models</span></a>
          ...
          <div style={{ flexShrink: 0, padding: '4px 24px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 4 }}>
              {agents.map((a) => <RosterCard key={a.id} a={a} todo={a.taskId ? todos.find((t) => t.id === a.taskId) : null} onOpen={actions.open} />)}
            </div>
          </div>
