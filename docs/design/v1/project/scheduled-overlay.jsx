/* Brain2 Console — Scheduled runs overlay.
   A multi-day timeline strip that scrolls horizontally beneath a fixed
   selector lens — scroll left to reach Jun 8 and earlier, and the runs
   underneath move with it. The agenda list shows every run currently under
   the lens. Header date is clickable → calendar to jump to a date. Upcoming-
   soon runs flash on the timeline. Each row has an on/off switch (stops
   subsequent runs) plus a ⋯ menu: Skip this run / Delete schedule.
   Exports: ScheduledRunsOverlay */
...
// Date model — a window of days around today (Jun 9, 2026 = index 5).
...
for (let d = RANGE_START_DAY; d <= RANGE_END_DAY; d++) {
  ...
  DAYS.push({ idx: DAYS.length, day: d, wd: date.getDay(), weekday: WD_SHORT[date.getDay()],
...
function cronFields(expr) {
  ...
  return { dom: p[2], dow: p[3 + 1] || p[4] || '*' };
}
// Does a schedule fire on this calendar day?
function fires(sched, dayMeta) {
...
function buildOccurrences(schedules) {
  const out = [];
  DAYS.forEach((dm) => {
    schedules.forEach((s) => {
      ...
      out.push({
        ...
        time: s.time, absMin: dm.idx * 1440 + s.time,
        x: xOf(dm.idx, s.time),
      });
    });
  });
...
// ── Small atoms ───────────────────────────────────────────────────────────
function SchedToggle({ checked, onChange }) {
  return (
    <button onClick={onChange} title={checked ? 'Schedule on — runs will fire' : 'Schedule off — subsequent runs paused'}
      style={{ position:'relative', width:38, height:22, borderRadius:11, border:'none', cursor:'pointer',
        background: checked ? 'var(--accent)' : 'var(--surface-3)', transition:'background 0.15s', flexShrink:0, padding:0 }}>
      <span style={{ position:'absolute', top:3, left: checked ? 19 : 3, width:16, height:16,
        borderRadius:'50%', background:'#fff', transition:'left 0.15s', display:'block' }} />
    </button>
  );
}

function soBtn(extra) {
  return { width:32, height:32, borderRadius:8, border:'1px solid var(--border)', background:'transparent',
    display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0, ...extra };
}

function CadenceChip({ sched }) {
...
function CalendarPopover({ focusDay, onPick, onClose }) {
  const [mode, setMode]   = React.useState('days');         // 'days' | 'months' | 'years'
  const [vYear, setVYear] = React.useState(DATA_YEAR);
  const [vMon, setVMon]   = React.useState(DATA_MONTH);
  const [yearBase, setYearBase] = React.useState(DATA_YEAR - 4); // first cell of the 12-year page

  const isDataMonth = vYear === DATA_YEAR && vMon === DATA_MONTH;
  const inRange = (d) => isDataMonth && d >= RANGE_START_DAY && d <= RANGE_END_DAY;
  ...
  // Header navigation differs per mode.
  const stepMonth = (dir) => {
    let m = vMon + dir, y = vYear;
    if (m < 0)  { m = 11; y--; }
    if (m > 11) { m = 0;  y++; }
  ...
  return (
    <Popover onClose={onClose} style={{ top:'calc(100% + 8px)', right:0, width:268, padding:14 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
        <button onClick={() => nav(-1)} style={soBtn({ width:28, height:28, color:'var(--fg-muted)' })}><Icon name="chevLeft" size={14} color="var(--fg-muted)" /></button>
        <button onClick={onHeaderClick}
          style={{ height:28, padding:'0 10px', display:'inline-flex', alignItems:'center', gap:6, border:'none', background:'transparent', cursor: mode === 'years' ? 'default' : 'pointer',
            fontSize:13.5, fontWeight:700, fontFamily:'var(--display-font)', letterSpacing:'var(--display-track)', color:'var(--fg)', borderRadius:7 }}>
          {headerLabel}
          {mode !== 'years' && <Icon name="chevDown" size={12} color="var(--fg-muted)" />}
        </button>
        <button onClick={() => nav(1)} style={soBtn({ width:28, height:28, color:'var(--fg-muted)' })}><Icon name="chevRight" size={14} color="var(--fg-muted)" /></button>
      </div>
        ...
        <React.Fragment>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:2 }}>
            {cells.map((d, i) => {
              ...
              return (
                <button key={d} disabled={!ok}
                  onClick={() => { if (ok) { onPick(d - RANGE_START_DAY); onClose(); } }}
                  style={{ position:'relative', height:32, borderRadius:7, border:'none', cursor: ok ? 'pointer' : 'default',
      ...
      {mode === 'months' && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6 }}>
          {CAL_MONTH_SHORT.map((m, i) => {
            ...
            const cur     = i === vMon;
            return (
              <button key={m} onClick={() => { setVMon(i); setMode('days'); }}
                style={{ position:'relative', height:42, borderRadius:8, cursor:'pointer',
                  border:`1px solid ${cur ? 'var(--accent)' : 'var(--border)'}`,
                  background: cur ? 'var(--accent-soft)' : 'transparent',
                  color: cur ? 'var(--accent)' : 'var(--fg)', fontFamily:'var(--ui-font)', fontSize:12.5, fontWeight:600 }}>
                {m}
      ...
      {mode === 'years' && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6 }}>
          {Array.from({ length: 12 }, (_, i) => yearBase + i).map((y) => {
            ...
            const cur     = y === vYear;
            return (
              <button key={y} onClick={() => { setVYear(y); setMode('months'); }}
                style={{ position:'relative', height:42, borderRadius:8, cursor:'pointer',
                  border:`1px solid ${cur ? 'var(--accent)' : 'var(--border)'}`,
                  background: cur ? 'var(--accent-soft)' : 'transparent',
                  color: cur ? 'var(--accent)' : 'var(--fg)', fontFamily:'var(--mono-font)', fontSize:12.5, fontWeight:600 }}>
                {y}
                {hasData && !cur && <span style={{ position:'absolute', bottom:6, left:'50%', transform:'translateX(-50%)', width:4, height:4, borderRadius:'50%', background:'var(--accent)' }} />}
              </button>
            );
  ...
  return (
    <div style={{ position:'relative', flexShrink:0 }}>
      <button onClick={(e) => { e.stopPropagation(); setOpen((o) => !o); }} title="More"
        style={soBtn({ width:30, height:30, border:`1px solid ${open ? 'var(--border-strong)' : 'transparent'}`, color:'var(--fg-muted)', background: open ? 'var(--surface-2)' : 'transparent' })}>
        <Icon name="more" size={16} color="var(--fg-muted)" />
      </button>
      {open && (
        <Popover onClose={() => setOpen(false)} style={{ top:'calc(100% + 4px)', right:0, width:212, padding:6 }}>
          ...
          {isPast ? (
            ...
            <button onClick={(e) => { e.stopPropagation(); (isSkipped ? onUnskip : onSkip)(); setOpen(false); }}
              style={menuBtn()}>
...
function menuBtn(color) {
  return { display:'flex', alignItems:'center', gap:10, width:'100%', padding:'9px 10px', border:'none',
...
function TimelineStrip({ occ, nextKey, soonKeys, scrollLeft, setScrollLeft, lens, setLens, cw, setCw, statusOf }) {
  ...
  const [drag, setDrag] = React.useState(null); // { mode:'pan'|'start'|'end', startX, base }
  ...
  React.useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      else if (drag.mode === 'start') setLens([Math.max(0, Math.min(drag.base + dx, lens[1] - 60)), lens[1]]);
      else                            setLens([lens[0], Math.min(cw, Math.max(drag.base + dx, lens[0] + 60))]);
    };
      ...
      <div style={{ position:'relative' }}>
        <div ref={scrollRef} onWheel={onWheel}
          ...
          <div style={{ position:'relative', width:TOTAL_W, height:96, transform:`translateX(${-scrollLeft}px)`, willChange:'transform' }}>
            {/* Day columns: separators, labels, hour ticks */}
            {DAYS.map((dm) => {
              const isToday = dm.idx === TODAY_IDX;
              const isPast  = dm.idx < TODAY_IDX;
              return (
                <div key={dm.idx} style={{ position:'absolute', top:0, bottom:0, left:dm.idx * DAY_W, width:DAY_W,
                  borderLeft:'1px solid var(--border)', background: isToday ? 'var(--accent-soft)' : isPast ? 'var(--surface-2)' : 'transparent' }}>
                  <span style={{ position:'absolute', top:6, left:DAY_PAD, fontSize:11, fontWeight:700, fontFamily:'var(--mono-font)',
            ...
            {DAYS.map((dm) => HOUR_TICKS.map((h) => (
              <span key={`${dm.idx}-${h}`} style={{ position:'absolute', bottom:6, left:xOf(dm.idx, h*60), transform:'translateX(-50%)',
                fontSize:9.5, color:'var(--fg-faint)', fontFamily:'var(--mono-font)' }}>{String(h).padStart(2,'0')}</span>
            )))}
            ...
            {occ.map((o) => {
              ...
              // Ran runs keep their format colour (just faded); off/skipped go grey.
              const grey    = st.kind === 'off' || st.kind === 'skipped';
              const size    = (isNext || soon) ? 13 : 10;
              return (
                <div key={o.key} title={`${dm_label(o)} ${hhmm(o.time)} · ${o.sched.title}`}
                  style={{ position:'absolute', bottom:24, left:o.x, transform:'translateX(-50%)', zIndex:3, display:'flex', flexDirection:'column', alignItems:'center' }}>
                  <span style={{ position:'relative', width:size, height:size, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    ...
                    <span style={{ position:'relative', width:size, height:size, borderRadius:'50%', border:'2px solid var(--surface)',
                      background: grey ? 'var(--surface-3)' : f.c,
                      opacity: grey ? 0.55 : st.kind === 'ran' ? 0.42 : 1,
                      boxShadow: isNext && !soon ? `0 0 0 4px ${f.s}` : 'none' }} />
                  </span>
        ...
        {cw > 0 && (
          <div style={{ position:'absolute', top:0, bottom:0, left:0, right:0, pointerEvents:'none' }}>
            ...
            {/* band */}
            <div style={{ position:'absolute', top:0, bottom:0, left:lens[0], width:lens[1]-lens[0],
              border:'1.5px solid var(--accent)', borderRadius:10, boxShadow:'0 0 0 1px rgba(0,0,0,0.2)' }} />
            {/* handles */}
            {[['start', lens[0]], ['end', lens[1]]].map(([side, pos]) => (
              <div key={side} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); setDrag({ mode:side, startX:e.clientX, base: pos }); }}
                style={{ position:'absolute', top:0, bottom:0, left:pos - 7, width:14, cursor:'ew-resize', pointerEvents:'auto',
...
const dm_label = (o) => o.dayIndex === TODAY_IDX ? 'Today' : o.day.label;
...
function AgendaRow({ occ, status, isNext, onToggle, onEdit, onRunNow, onSkip, onUnskip, onDelete, border }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:13, padding:'11px 10px', margin:'0 -10px', borderRadius:9,
      borderTop: border ? '1px solid var(--border)' : 'none',
      background: isNext ? 'var(--accent-soft)' : 'transparent', opacity: status.kind === 'off' ? 0.55 : 1 }}>
      {/* Time */}
      <span style={{ width:48, flexShrink:0, fontSize:13.5, fontWeight:700, fontFamily:'var(--mono-font)',
        color: isNext ? 'var(--accent)' : dim ? 'var(--fg-faint)' : 'var(--fg)',
        textDecoration: struck ? 'line-through' : 'none' }}>
        {hhmm(occ.time)}
      </span>
      {/* Format icon */}
      <span style={{ width:32, height:32, borderRadius:8, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center',
        background: dim ? 'var(--surface-2)' : f.s, color: dim ? 'var(--fg-faint)' : f.c }}>
        <Icon name={f.icon} size={15} />
      </span>
      {/* Title + meta */}
      <span style={{ flex:1, minWidth:0 }}>
        <span style={{ display:'block', fontSize:13.5, fontWeight:600, color:'var(--fg)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis',
          textDecoration: struck ? 'line-through' : 'none' }}>{occ.sched.title}</span>
      ...
      {/* Overflow menu */}
      <RowMenu isSkipped={status.kind === 'skipped'} isPast={status.kind === 'ran'}
...
// ── Main overlay ──────────────────────────────────────────────────────────
function ScheduledRunsOverlay({ onClose }) {
  const [schedules, setSchedules] = React.useState(SCHEDULES);
  const [skipped, setSkipped]     = React.useState(() => new Set());
  const [deleted, setDeleted]     = React.useState(() => new Set());
  const [scrollLeft, setScrollLeft] = React.useState(Math.max(0, NOW_X - 360));
  const [lens, setLens]           = React.useState([null, null]);
  ...
  const occ = React.useMemo(() => {
    ...
    // Deleted schedule: drop its upcoming runs, keep ones that already ran.
    return all.filter((o) => !(deleted.has(o.sched.id) && o.absMin > NOW_ABS));
  }, [schedules, deleted]);

  // Status resolver for an occurrence.
  // Order matters: a run that already fired reads "Ran"; otherwise a disabled
  // schedule reads "Off" and overpowers a per-run "Skipped".
  const statusOf = (o) => {
  ...
  const lensReady = lens[0] !== null && cw > 0;
  const loX = lensReady ? scrollLeft + lens[0] : 0;
  const hiX = lensReady ? scrollLeft + lens[1] : TOTAL_W;
  const visible = occ.filter((o) => o.x >= loX && o.x <= hiX);

  const mLo = xToMoment(loX), mHi = xToMoment(hiX);
  ...
  // Actions.
  const toggleEnabled = (id) => setSchedules((ss) => ss.map((s) => s.id === id ? { ...s, enabled:!s.enabled } : s));
  const skip   = (key) => setSkipped((s) => new Set(s).add(key));
  const unskip = (key) => setSkipped((s) => { const n = new Set(s); n.delete(key); return n; });
  const del    = (id)  => setDeleted((d) => new Set(d).add(id));
  const editSched = (id) => { const s = schedules.find((x) => x.id === id); setNotice({ icon:'pencil', text:`Opening editor for “${s ? s.title : 'schedule'}”…` }); };
  const runNow    = (key) => { const o = occ.find((x) => x.key === key); setNotice({ icon:'zap', text:`Queued “${o ? o.sched.title : 'run'}” to run now` }); };

  // Scroll helpers (clamp to content).
  const centreLens = () => lensReady ? (lens[0] + lens[1]) / 2 : cw / 2;
  const scrollTo   = (x) => setScrollLeft(Math.max(0, Math.min(TOTAL_W - cw, x)));
  const nudgeDay   = (dir) => scrollTo(scrollLeft + dir * DAY_W);
  const jumpToDay  = (dayIndex) => scrollTo(dayIndex * DAY_W + DAY_W / 2 - centreLens());
  ...
  return (
    <div onClick={onClose}
      style={{ position:'fixed', inset:0, zIndex:100, background:'rgba(8,10,13,0.62)', backdropFilter:'blur(2px)', WebkitBackdropFilter:'blur(2px)',
        display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'5vh 16px 16px' }}>
      <div onClick={(e) => e.stopPropagation()}
        style={{ width:'100%', maxWidth:820, maxHeight:'90vh', display:'flex', flexDirection:'column', position:'relative',
          background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16,
          boxShadow:'0 30px 90px rgba(0,0,0,0.55)', overflow:'hidden', fontFamily:'var(--ui-font)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, padding:'16px 22px', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
          ...
          <div style={{ display:'inline-flex', alignItems:'center', gap:4, position:'relative' }}>
            <button onClick={() => nudgeDay(-1)} style={soBtn({ color:'var(--fg-muted)' })}><Icon name="chevLeft" size={15} color="var(--fg-muted)" /></button>
            ...
            <button onClick={() => nudgeDay(1)} style={soBtn({ color:'var(--fg-muted)' })}><Icon name="chevRight" size={15} color="var(--fg-muted)" /></button>
            {calOpen && <CalendarPopover focusDay={focusDayMeta ? focusDayMeta.day : TODAY_DAY} onPick={jumpToDay} onClose={() => setCalOpen(false)} />}
          </div>
          <button onClick={onClose} style={soBtn({ color:'var(--fg-muted)' })}><Icon name="x" size={16} color="var(--fg-muted)" /></button>
        </div>

        {/* Timeline strip */}
        <TimelineStrip occ={occ} nextKey={nextKey} soonKeys={soonKeys}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 22px 4px', flexShrink:0 }}>
          ...
          <button onClick={() => scrollTo(NOW_X - centreLens())}
            ...
            : groups.map((g) => (
                <div key={g.dayIndex}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 0 6px' }}>
                    <span style={{ fontSize:10.5, fontWeight:700, letterSpacing:'0.06em', textTransform:'uppercase',
                      color: g.dayIndex === TODAY_IDX ? 'var(--accent)' : 'var(--fg-faint)' }}>
                      {g.dayIndex === TODAY_IDX ? 'Today' : g.day.label}
                    </span>
                    <span style={{ flex:1, height:1, background:'var(--border)' }} />
                  </div>
                  {g.items.map((o, i) => (
                    <AgendaRow key={o.key} occ={o} status={statusOf(o)} isNext={o.key === nextKey}
        ...
        {notice && (
          <div style={{ position:'absolute', left:'50%', bottom:64, transform:'translateX(-50%)', zIndex:20,
            display:'inline-flex', alignItems:'center', gap:9, padding:'10px 15px', borderRadius:10,
            background:'var(--fg)', color:'var(--bg)', fontSize:12.5, fontWeight:600, fontFamily:'var(--ui-font)',
            boxShadow:'0 12px 34px rgba(0,0,0,0.4)', whiteSpace:'nowrap' }}>
...
Object.assign(window, { ScheduledRunsOverlay });
