...
def test_index_lists_pages_by_zone(tmp_path):
    s = _seed()
    s.upsert_vault_page(VaultPage(tenant_id="t1", project_id="p1", path="wiki/concepts/attention.md", zone="wiki",
                                  topic="attention", tldr="how transformers focus", content_hash="h", mtime=1, source_type="wiki"))
    s.upsert_vault_page(VaultPage(tenant_id="t1", project_id="p1", path="wiki/entities/karpathy.md", zone="wiki",
                                  topic="karpathy", tldr="AI educator", content_hash="h", mtime=1, source_type="wiki"))
    s.upsert_vault_page(VaultPage(tenant_id="t1", project_id="p1", path="static/code-of-conduct.pdf", zone="static",
                                  topic="code-of-conduct", tldr="company policy", content_hash="h", mtime=1, source_type="static"))
    out = generate_index_md(s, "t1", "p1")
    assert "# Index" in out
...
def test_index_skips_sources_zone(tmp_path):
    s = _seed()
    s.upsert_vault_page(VaultPage(tenant_id="t1", project_id="p1", path="wiki/sources/x.md", zone="wiki", topic="x", tldr="t",
                                  content_hash="h", mtime=1, source_type="wiki"))
    s.upsert_vault_page(VaultPage(tenant_id="t1", project_id="p1", path="wiki/concepts/y.md", zone="wiki", topic="y", tldr="t",
                                  content_hash="h", mtime=1, source_type="wiki"))
    out = generate_index_md(s, "t1", "p1")
