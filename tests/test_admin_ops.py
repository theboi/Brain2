...
def _setup():
    ...
    ws = s.create_workspace("t1", "Default")
    pw = PasswordManager(s)
    return s, pw, ws.workspace_id
...
def test_create_user_creates_and_sets_password():
    ...
    assert out["workspace_role"] == "member"
    # Admin-seeded password must force change on first login
    row = s._conn.execute(
...
def test_create_user_rejects_missing_workspace_for_non_owner_role():
    s, pw, ws_id = _setup()
    with pytest.raises(Conflict, match="workspace_id required"):
        make_create_user(s, pw)(_ctx(), {
            "email": "x@t1.com", "password": "pw", "display_name": "X", "role": "member"})
