...
ATTENTION_PAGE = VaultPage(project_id="p1", path="wiki/concepts/attention.md", zone="wiki",
                            topic="attention", content_hash="abc123", mtime=1700000000)
TRANSFORMER_PAGE = VaultPage(project_id="p1", path="wiki/concepts/transformers.md", zone="wiki",
...
def test_replace_links_and_get_outgoing():
    ...
    s.replace_links_for_source("p1", "wiki/concepts/transformers.md", links)
...
def test_replace_links_replaces_old():
    ...
    s.replace_links_for_source("p1", "wiki/concepts/transformers.md", old_links)
    new_links = [
        VaultLink(project_id="p1", source_path="wiki/concepts/transformers.md",
                  target_topic="attention", target_zone="wiki"),
    ]
    s.replace_links_for_source("p1", "wiki/concepts/transformers.md", new_links)
    outgoing = s.get_outgoing_links("p1", "wiki/concepts/transformers.md")
    assert len(outgoing) == 1
