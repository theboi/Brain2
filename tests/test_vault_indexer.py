...
def test_index_file_creates_vault_page_row(tmp_path):
    ...
    index_file(s, "t1", "p1", root, p)
    page = s.get_vault_page("t1", "p1", "wiki/concepts/attention.md")
...
def test_index_file_extracts_links(tmp_path):
    ...
    index_file(s, "t1", "p1", root, src)

    links = s.get_outgoing_links("t1", "p1", "wiki/concepts/attention.md")
...
def test_reindex_path_updates_single_file(tmp_path):
    ...
    write_text_atomic(root / "wiki" / "concepts" / "b.md", "# B\n")
    reindex_vault(s, "t1", "p1", root)
    assert s.get_vault_page_by_topic("t1", "p1", "a") is not None

    # Edit a.md on disk; reindex only that path.
    write_text_atomic(root / "wiki" / "concepts" / "a.md", "# A v2\n")
    reindex_path(s, "t1", "p1", root, "wiki/concepts/a.md")
    a = s.get_vault_page_by_topic("t1", "p1", "a")
    assert "v2" in (root / "wiki" / "concepts" / "a.md").read_text()
    # And b.md must be untouched.
    assert s.get_vault_page_by_topic("t1", "p1", "b") is not None
...
def test_reindex_path_deletes_when_file_missing(tmp_path):
    ...
    reindex_path(s, "t1", "p1", root, "wiki/a.md")
    assert s.get_vault_page_by_topic("t1", "p1", "a") is None
