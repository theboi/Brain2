/*
 * Mock data for the Scheduled runs overlay (visual port of scheduled-data.js).
 *   time:          queue time in minutes from midnight (no duration).
 *   enabled:       on/off. Once a run fires it leaves scheduling; no pause.
 ...
 *
 * Placeholder data — the overlay will be wired to `schedules:list` later.
 * Anchored to "now" = 14:18 on Tue Jun 9, 2026.
 */

export type SchedFormat = 'doc' | 'deck' | 'video';
export type CadenceId = 'daily' | 'weekdays' | 'weekly' | 'monthly' | 'quarterly' | 'custom';
...
export const SCHED_NOW = 14 * 60 + 18; // 14:18 on Jun 9, 2026
export const SCHED_DAY = { weekday: 'Tuesday', date: 'Jun 9, 2026', short: 'Jun 9' };
